...
namespace graphics
{
	...
	void Shader::SetUniform2f(const std::string& name, const math::vec2<float>& value)
	{
		GLCall(glUniform2f(GetUniformLocation(name), value.x, value.y));
	}

	void Shader::SetUniform3f(const std::string& name, const math::vec3<float>& value)
	{
		GLCall(glUniform3f(GetUniformLocation(name), value.x, value.y, value.z));
	}

	void Shader::SetUniform4f(const std::string& name, const math::vec4<float>& value)
	{
		GLCall(glUniform4f(GetUniformLocation(name), value.x, value.y, value.z, value.w));
	}

	void Shader::SetUniformMatrix4f(const std::string& name, const math::mat4<float>& data)
	{
